...
export default function ElderlySimulationsPage() {
    ...
    const loadDrills = async () => {
        ...
        try {
            const res = await fetch('/api/simulations?age_group=elderly');
            if (res.ok) {
    ...
    if (currentView === 'active-call') {
        return (
            <div className="page-content">
                <VoiceSimulator ageGroup="elderly" onComplete={handleCallComplete} />
            </div>
        );
    }
        ...
        return (
            <div className="page-content">
                <button
                    ...
                    className="mb-4 flex items-center gap-2 text-gray-600 hover:text-gray-800 text-lg"
                >
                    ← Back
                </button>
        ...
        return (
            <div className="page-content space-y-4">
                <button
                    ...
                    className="mb-2 text-gray-600 hover:text-gray-800 text-lg flex items-center gap-2"
                >
                    ← Back to Drills
                </button>

                <div className="bg-gradient-to-br from-blue-50 to-cyan-50 border border-cyan-100 rounded-3xl p-6 shadow-lg">
                    <p className="text-sm text-gray-500 mb-2 uppercase font-bold tracking-wide">Scenario</p>
                    <h2 className="text-3xl font-bold text-gray-800 mb-2">{selectedDrill.title}</h2>
                    <p className="text-gray-700 mb-4 text-lg">{selectedDrill.description}</p>
                    {scenario.prompt && (
                        <div className="bg-white rounded-2xl p-5 shadow-inner border border-cyan-100">
                            <p className="text-gray-800 text-lg leading-relaxed">{scenario.prompt}</p>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-3">
                    {options.map((opt: any) => (
                        ...
                        >
                            ...
                            <div>
                                <p className="font-semibold text-gray-800 text-lg">{opt.label || opt.text}</p>
                ...
                <button
                    ...
                    className="w-full mt-2 bg-teal-500 hover:bg-teal-600 disabled:bg-gray-300 text-white rounded-full py-3 text-xl font-bold shadow-[0_6px_0_0_rgba(6,182,212,0.4)] hover:shadow-[0_4px_0_0_rgba(6,182,212,0.4)] active:translate-y-0.5 transition-all"
                >
        ...
        return (
            <div className="page-content space-y-4 text-center">
                ...
                <h2 className="text-3xl font-bold text-gray-800">
                    {submitResult.success ? 'Excellent choice!' : 'Let’s review that'}
                </h2>
                <p className="text-gray-600 text-lg">{submitResult.feedback || (submitResult.success ? 'You chose the safest action.' : 'That option was risky. Pick the safer choice.')}</p>
    ...
    if (currentView === 'others') {
        return (
            <div className="page-content">
                <button
                    onClick={() => setCurrentView('menu')}
                    className="mb-6 text-gray-600 hover:text-gray-800 text-lg"
                >
                    ← Back to Menu
                </button>

                <h2 className="text-3xl font-bold text-gray-800 mb-4">More Drills</h2>

                {drillsLoading ? (
                    <div className="bg-white rounded-2xl p-8 text-center shadow-lg">
                        <p className="text-gray-600 text-lg">Loading drills...</p>
                    </div>
                ) : drills.length === 0 ? (
                    <div className="bg-white rounded-2xl p-8 text-center shadow-lg">
                        <p className="text-gray-600 text-lg">No drills published yet.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 gap-4">
                        {drills.map((drill) => (
                            <div
                                key={drill.id}
                                className="bg-gradient-to-br from-blue-50 to-cyan-50 border border-cyan-100 rounded-3xl p-6 shadow-md hover:shadow-xl transition-all"
                            >
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        <h3 className="text-2xl font-bold text-gray-800">{drill.title}</h3>
                                        <p className="text-base text-gray-700 mt-1">{drill.description}</p>
